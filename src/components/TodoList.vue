  ...
  <div class="wrap">
        ...
        <div v-for="item in todoList" :key="item.id" class="list">
          <div v-if="!item.isFinished" class="item-container list" @click="EditTodo">
            <div class="center item-left">
              <input type="checkbox" class="item__check" :checked="item.isFinished ? 'checked' : ''" @click="ToggleTodo" v-model="item.id"/>
    ...
    <div class="donelist">
      <!-- <div
        ...
        data-type="done"
      >
        <i class="[spread done.collapse ? 'collapse-rotate' : '']"></i>已完成
        <i class="done-icon"></i>
        {{doneNum}}
      </div>
      <block v-if="done.isShowed" wx:for={{todoList}} wx:key="id" data={{item}} class="list">
        <div
